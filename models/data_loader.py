...
class Batch(object):
    # src, labels, segs, clss, tgt_idxs, decoder_tgt_idxs, topic_pro
    ...
    def __init__(self, data=None, device=None,  is_test=False):
        ...
        if data is not None:
            ...
            if not is_test:
                ...
                decoder_tgt_idxs = torch.tensor(self._pad(pre_decoder_tgt_idxs, 0))
...
def batch(data, batch_size, is_test):
...
class DataIterator(object):
    # batch_size: the total number of src word.
    ...
    def preprocess(self, ex, is_test):
        ...
        src_txt = ex['src_txt']
        tgt_txt = ex['tgt_txt']
        topic_pro = ex['topic_pro']
        if not is_test:
            tgt_idxs = ex['tgt_idxs']
            decoder_tgt_idxs = ex['decoder_tgt_idxs']
            # logger.info('----------------------------------------------------------------------')
            # logger.info('tgt_idxs {}'.format(tgt_idxs))
            # logger.info('decoder_tgt_idxs {}'.format(decoder_tgt_idxs))
            # logger.info('-----------------------------------------------------------------------')
        if (is_test):
            return src, labels, segs, clss, src_txt, tgt_txt, topic_pro
        else:
            return src, labels, segs, clss, tgt_idxs, decoder_tgt_idxs, topic_pro
    ...
    def create_batches(self):
        ...
        for buffer in self.batch_buffer(data, self.batch_size * 50):
            ...
            p_batch = batch(p_batch, self.batch_size, self.is_test)
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                if self._iterations_this_epoch > idx or len(minibatch) == 0:
                    continue
                self.iterations += 1
